...
function BarsHorizontal(chartData, canvas_id) {
        var tip = d3.tip()
        .attr('class', 'd3-tip')
        .offset([-10, 0])
        .html(function(d) {
        ...
        var svg = d3.select("#"+canvas_id).append("svg")
            ...
            .attr("viewBox", "-80 0 " + outerWidth + " " + outerHeight); 
        ...
        xScale.domain([0, d3.max(chartData, function (d){ return d[xColumn]; })]);
        yScale.domain(       chartData.map( function (d){ return d[yColumn]; }));
        xAxisG.call(xAxis);
        yAxisG.call(yAxis);
